...
void integrate_vv(int n_steps, int reuse_forces, bool i_know_what_im_doing) {
...
int python_integrate(int n_steps, bool recalc_forces, bool reuse_forces_par,
  ...
  for (int i = 0; i < n_steps;) {
    ...
    if (mpi_integrate(steps, reuse_forces, i_know_what_im_doing))
      return ES_ERROR;
  ...
  if (n_steps == 0) {
    if (mpi_integrate(0, reuse_forces, i_know_what_im_doing))
      return ES_ERROR;
  }
